  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const metaRows: [string, string][] = [
        ...
        ['Order Date', fmt(invoice.orderDate)],
        ['Due Date', fmt(invoice.deliveryDate)],
      ]
      doc.setFontSize(7)
      metaRows.forEach(([lbl, val], i) => {
        const ry = MY + i * 13
        doc.setFont('helvetica', 'normal'); doc.setTextColor(...G100); doc.text(lbl, MX, ry)
      ...
      const BW = CW * 0.55
      doc.setFont('helvetica', 'bold'); doc.setFontSize(6); doc.setTextColor(...S300)
      ...
      autoTable(doc, {
        ...
        body: invoice.items.map((item, idx) => [
          String(idx + 1), item.name, String(item.quantity),
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...S300)
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...G900)
      doc.text('[OK]  ' + invoice.paymentMethod, PAY_X + 8, y + 30)
      doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...S300)
      doc.text('TRACKING NUMBER', PAY_X, y + 54)
      doc.setFont('courier', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...S900)
      doc.text(invoice.trackingNumber, PAY_X, y + 66)
      doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...S300)
      doc.text('TRACK URL', PAY_X, y + 82)
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(...G700)
