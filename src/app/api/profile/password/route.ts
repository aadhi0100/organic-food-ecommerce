...
export async function POST(request: Request) {
  try {
    ...
    const currentUser = await UserStore.findById(sessionUser.id)
    ...
    if (currentUser.passwordHash) {
      ...
      if (!verified) return NextResponse.json({ error: 'Current password is incorrect' }, { status: 400 })
    }

    const updated = await UserStore.setPassword(currentUser.id, parsed.data.newPassword)
    if (!updated) return NextResponse.json({ error: 'Unable to update password' }, { status: 500 })
